...
def _category_codes(session: Session, product_id: int) -> list[tuple[str, bool]]:
    rows = session.execute(
        select(Category.upgates_code, ProductCategory.primary_yn)
    ...
    return [(code, primary) for code, primary in rows if code]


def render_products(session: Session, rules: dict) -> bytes:
    ...
    products = session.execute(select(Product).where(Product.active.is_(True))).scalars()
    for p in products:
        ...
        cats = _category_codes(session, p.id)
        if cats:
...
def run_export(session: Session, out_dir) -> dict:
    ...
    xml = render_products(session, rules)
    ...
    count = len(ET.fromstring(xml).findall("PRODUCT"))
