...
for (category_name, parent_id), details in categories.items():
    ...
    code_value = f"{domain_name}{parent_name}|{unidecode(category_name.lower().replace(' ', '-'))}|{details['id']}"
...
# Hlavičky
ws.append(["Název kategorie", "Kód kategorie", "ID kategorie", "ID nadřazené kategorie"])
