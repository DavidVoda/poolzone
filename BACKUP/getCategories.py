...
for (category_name, parent_id), details in categories.items():
    ...
    # CODE obsahuje doménu, category_name bez diakritiky, malými písmeny a s pomlčkami, plus ID kategorie
    code_value = f"{domain_name}-{unidecode(category_name.lower().replace(' ', '-'))}-{details['id']}"
...
# Ulož výsledný XML
output_file = "upgates_categories.xml"
