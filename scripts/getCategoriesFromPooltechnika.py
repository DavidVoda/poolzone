...
for item in root.findall('SHOPITEM'):
    ...
    if category_text is not None:
        category_text_original = category_text.text.strip()
    if category_text is not None and category_text.text.strip():
        ...
        key = ("Další", None)
        if key not in categories:
            categories[key] = {"id": id_counter, "parent_id": None, "name": "Další", "old_name": category_text_original}
            id_counter += 1
    # print(category_text_original)
...
for (category_name, parent_id), details in categories.items():
    ...
    excel_data.append([
        ...
        code_value,
        details["id"],
        details["parent_id"] if details["parent_id"] else ""
    ])

print(f"XML a XLSX soubor pro import a správu kategorií byl úspěšně vytvořen.")
...
print(f"XML soubor uložen.")

# Generování Excelu
excel_file = f"{domain_name}_categories.xlsx"
wb = openpyxl.Workbook()
ws = wb.active
ws.title = "Kategorie"

# Hlavičky
ws.append(["Název původní", "Název kategorie", "Kód kategorie", "ID kategorie", "ID nadřazené kategorie"])
...
print(f"XLSX soubor uložen.")

print(f"Feed byl úspěšně zpracován. Výstupní soubor XML: {output_file}, Excel: {excel_file}.")
